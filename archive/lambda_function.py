...
def parse_crypto_commands(text, api_token):
    ...
    if '!price' == string[0]:
        ...
        try:
            req = requests.get(url='http://pro-api.coinmarketcap.com/v1/cryptocurrency/quotes/latest?symbol=' +
                               crypto_db[string[1].lower()], headers={'X-CMC_PRO_API_KEY': api_token}, verify=False)
...
def create_crypto_db(api_token):
    ...
    req = requests.get(
        url='http://pro-api.coinmarketcap.com/v1/cryptocurrency/listings/latest?limit=5000', headers=headers, verify=False)
    all_coins = req.json()
    crypto_db = {}
...
def create_coin_output(coin):
    ...
    coin_output2 = "```{:20s}\t${:.2f}\n".format(
        "Price USD", float(coin['quote']['USD']['price']))
    coin_output3 = "{:20s}\t${:.2f}\n".format(
        "Market Cap", float(coin['quote']['USD']['market_cap']))
    coin_output4 = "{:20s}\t{:.2f}%\n".format(
        "Change 1hr", float(coin['quote']['USD']['percent_change_1h']))
    coin_output5 = "{:20s}\t{:.2f}%\n".format(
        "Change 24hr", float(coin['quote']['USD']['percent_change_24h']))
    coin_output6 = "{:20s}\t{:.2f}%\n```".format(
        "Change 7d", float(coin['quote']['USD']['percent_change_7d']))
